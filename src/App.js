...
const App = () => {
  ...
  const [showAddNotificationPopup, setShowAddNotificationPopup] = useState(false);

// Fetch todos from the API when the component mounts
useEffect(() => {
  // Making a GET request to fetch todos from the JSONPlaceholder API
  ...
  return (
    <div className="App">
      ...
      {visibleTodos < todos.length && (
        <button className="show-more-button" onClick={showMoreTodos}>Show More</button>
      )}
      ...
      {showAddNotificationPopup && (
        <AddTodoNotificationPopup onClose={() => setShowAddNotificationPopup(false)} />
      )}
